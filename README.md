...
MineSafe AI가 동일 질문에 대해 답변 구조와 핵심 안전조치를 얼마나 일관되게 유지하는지 확인하기 위해 대표 질문 10개를 선정하고 각 질문을 5회씩 반복 실행했습니다. 총 50개 답변을 수집하고 같은 질문 내 답변 조합 100쌍의 유사도를 계산했습니다.

- 대표 질문 수: 10개
- 반복 답변 수: 50개
- pairwise 비교 수: 100개
- 전체 평균 유사성 지수: 100.00점
- consistency_level: 매우 높음
- 외부 LLM API 사용: 없음
- 계산 방식: `difflib.SequenceMatcher fallback`

유사성 지수가 높다는 것은 동일 질문 반복 입력 시 답변 구조와 핵심 안전조치가 안정적으로 유지된다는 의미입니다. 다만 이 지표는 정확도 평가가 아니라 응답 일관성을 확인하는 보조 지표이며, deterministic 안정형 RAG 답변에서는 유사성이 높게 나올 수 있습니다.
